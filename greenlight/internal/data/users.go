...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(len(password) <= 30, "password", "must not be more than 30 bytes long")
}
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	// useful sanity check to include here, but it's not a problem with the data provided
	// by the client. So rather than adding an error to the validation map we raise a panic instead.
